...
using namespace std;
const size_t buffer_f32_len = 16;
...
int main(int argc, char const *argv[]) {
    ...
    WGPUBuffer staging_buffer =
        wgpuDeviceCreateBuffer(wrapper.device, &staging_buffer_desc);

    WGPUBufferDescriptor storage_buffer_desc_x = {
        .label = {"storage_buffer_x", WGPU_STRLEN},
        .usage = WGPUBufferUsage_Storage | WGPUBufferUsage_CopyDst |
                 WGPUBufferUsage_CopySrc,
        .size = buffer_size,
        .mappedAtCreation = false,
    };
    WGPUBuffer storage_buffer_x =
        wgpuDeviceCreateBuffer(wrapper.device, &storage_buffer_desc_x);
    ...
    WGPUComputePipeline compute_pipeline =
        wgpuDeviceCreateComputePipeline(wrapper.device, &compute_desc);

    WGPUBindGroupLayout bind_group_layout =
        wgpuComputePipelineGetBindGroupLayout(compute_pipeline, 0);

    WGPUBindGroupEntry entries[3] = {(const WGPUBindGroupEntry){
        .binding = 0,
    ...
    WGPUBindGroup bind_group =
        wgpuDeviceCreateBindGroup(wrapper.device, &bind_group_desc);

    WGPUCommandEncoderDescriptor encoder_desc = {
        .label = {"command_encoder", WGPU_STRLEN},
    };
    WGPUCommandEncoder command_encoder =
        wgpuDeviceCreateCommandEncoder(wrapper.device, &encoder_desc);

    WGPUComputePassDescriptor compute_pass_desc = {
        .label = {"compute_pass", WGPU_STRLEN},
    };
    WGPUComputePassEncoder compute_pass_encoder =
        wgpuCommandEncoderBeginComputePass(command_encoder, &compute_pass_desc);
    ...
    wgpuComputePassEncoderDispatchWorkgroups(compute_pass_encoder, buffer_size,
                                             1, 1);
    wgpuComputePassEncoderEnd(compute_pass_encoder);
    wgpuComputePassEncoderRelease(compute_pass_encoder);

    wgpuCommandEncoderCopyBufferToBuffer(command_encoder, storage_buffer_x, 0,
    ...
    WGPUCommandBuffer command_buffer =
        wgpuCommandEncoderFinish(command_encoder, &command_buff_desc);
    ...
    float *buf =
        (float *)wgpuBufferGetMappedRange(staging_buffer, 0, buffer_size);
    assert(buf);
